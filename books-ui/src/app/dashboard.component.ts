...
export class DashboardComponent {
  ...
  protected readonly searchableColumns = computed(() =>
    this.columns().filter((column) => !column.primaryKey && !this.isSystemManagedColumn(column))
  );
  ...
  protected currentUserName(): string {
    return this.authState.currentUser()?.fullName ?? 'Account';
  }
  ...
  protected isBooleanColumn(column: BookColumnMetadata): boolean {
    return column.jdbcType.toLowerCase().includes('bool');
  }

  protected isDateColumn(column: BookColumnMetadata): boolean {
    const type = column.jdbcType.toLowerCase();
    return type.includes('date') && !type.includes('timestamp');
  }

  protected inputTypeFor(column: BookColumnMetadata): string {
  ...
  protected selectBook(book: BookRecord): void {
    ...
    for (const column of this.searchableColumns()) {
      ...
      } else if (this.isDateColumn(column) && value != null) {
        this.form.controls[column.name]?.setValue(String(value).slice(0, 10));
      } else {
        this.form.controls[column.name]?.setValue(value == null ? '' : String(value));
      }
    }
  ...
  private buildForm(columns: BookColumnMetadata[]): void {
    for (const column of columns) {
      if (!column.primaryKey && !this.isSystemManagedColumn(column)) {
  ...
  private castValue(column: BookColumnMetadata, value: string): string | number | boolean {
    if (this.isBooleanColumn(column)) {
      return value === 'true';
    }
  ...
  private isSystemManagedColumn(column: BookColumnMetadata): boolean {
